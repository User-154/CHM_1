...
CubicSpline::CubicSpline(const std::vector<double>& x_, const std::vector<double>& y_)
    : x(x_), y(y_) {
...
double CubicSpline::deriv(double X) const {
    int i = 0;
    for (int j = 0; j < n; j++) {
        if (X >= x[j] && X <= x[j + 1]) {
            i = j;
...
double CubicSpline::deriv2(double X) const {
    int i = 0;
    for (int j = 0; j < n; j++) {
        if (X >= x[j] && X <= x[j + 1]) {
            i = j;
