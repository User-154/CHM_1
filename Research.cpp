...
void Research::conductSplineResearch(double a, double b,
    ...
    for (int level = 0; level < 3; level++) {
        ...
        auto grid = GridGenerator::uniformGrid(a, b, n);
        ...
        // Строим сплайн
        CubicSpline spline(grid, values);
        ...
        for (int i = 0; i < test_points_count; i++) {
            ...
            double spline_func = spline.eval(x);
            double spline_deriv = spline.deriv(x);
            double spline_deriv2 = spline.deriv2(x);
    ...
    std::vector<double> h_values = { 0.7, 0.5, 0.1, 0.01, 0.005, 0.0025, 0.00125 };
